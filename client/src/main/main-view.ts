import {IActor, IMedia, ICharacter} from '../../../server/src/serverTypes.js';
    ...
    showActors(actors: IActor[]) {
        if (this.actorTemp && this.infoBoxA) {
            for (let i = 0; i < actors.length; i++) {
                ...
                if (rootDiv) {
                    let itemEl = rootDiv.querySelector('.name') as HTMLAnchorElement | null;
                    if (itemEl) {
        ...
        if (media.length > 0) {
            if (this.mediaTemp && this.infoBoxA) {
                for (let i = 0; i < media.length; i++) {
                    ...
                    let rootDiv = clone.firstElementChild as HTMLDivElement | null;
                    if (rootDiv) {
                        let itemEl = rootDiv.querySelector('.title') as HTMLAnchorElement | null;
                        if (itemEl) {
